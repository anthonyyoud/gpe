; $Id: vizit.pro,v 1.18 2006/11/14 15:08:04 mee Exp $
...
PRO vizit, object, xsize=xsz, ysize=ysz, colortable=coltab, $
       ...
       pVector=pVector, alpha=alpha, bboxcolor=bboxcolor, $
...
if (keyword_set(PNG) or keyword_set(EPS)) then begin
    default, drawbbox, 1
...
;  for bboxcol=minbboxpal,255 do begin
;    BBoxPal=round(bgcolor+(bboxcolor-bgcolor)*float(bboxcol+1)/(256.-minbboxpal))
;    if min(where (BBoxPal gt 255)) ge 0 then BBoxPal[where (BBoxPal gt 255)]=255
;    if min(where (BBoxPal lt 0)) ge 0 then BBoxPal[where (BBoxPal lt 0)]=0
;  endfor
;
...
if not (keyword_set(PNG) or keyword_set(EPS)) then oHolder->add, oPlotView
  oHolder->add, oTrack
;drawbbox=1
;drawbbox=0
;drawcontent=1
...
;              pVector:     pVector,     $
  ...
  if (keyword_set(PNG) or keyword_set(GIF) or keyword_set(JPG) or $
    keyword_set(EPS)) then begin
      ...
      if n_elements(tmat) eq 16 then begin
        ...
        if keyword_set(PNG) then begin
          ...
          device, /color, bits=8, /encapsulated, /isolatin1, $
            filename=frame_filename+'.eps', xsize=0.5*xsz/72., $
            ysize=0.5*ysz/72., /inches
          tvlct, r, g, b
          tv, image2D, xsize=0.5*xsz/72., ysize=0.5*ysz/72., /inches
          device, /close
          !p.font = -1
          set_plot, 'x'
        endif
