; $Id: aatrackball__define.pro,v 1.1 2005/10/09 11:28:01 mee Exp $
;
...
; CALLING SEQUENCE:
;	To initially create:
;		oTrackball = OBJ_NEW('aaTrackball', Center, Radius)
;
...
;
FUNCTION TRACKBALL_CONSTRAIN, pt, vec
    ; Project the point.
...
;
FUNCTION aaTRACKBALL::UPDATE, sEvent, TRANSFORM=transform, $
    ...
    0: BEGIN    ;Button press.
         ...
         ;aa IF (sEvent.press EQ self.mouse) THEN BEGIN
           ...
           IF (self.constrain NE 0) THEN BEGIN
               vec = [0.,0.,0.]
               vec[self.axis] = 1.0
               self.pt1 = TRACKBALL_CONSTRAIN( self.pt1, vec)
           ENDIF
           self.pt0 = self.pt1
           self.btndown = 1b
         ;aa ENDIF
    ...
    2: BEGIN    ;Button motion.
         IF (self.btndown EQ 1b) THEN BEGIN
           ...
           IF (self.constrain NE 0) THEN BEGIN
               vec = [0.,0.,0.]
               vec[self.axis] = 1.0
               pt1 = TRACKBALL_CONSTRAIN( pt1, vec)
           ENDIF
...
;
FUNCTION aaTRACKBALL::INIT, center, radius, AXIS=axis, CONSTRAIN=constrain, $
                          MOUSE=mouse
...
;          The default is 1b, for the left moust button.
;
PRO aaTRACKBALL::RESET, center, radius, AXIS=axis, CONSTRAIN=constrain, $
                      MOUSE=mouse
    IF (N_ELEMENTS(center) NE 2) THEN BEGIN
        PRINT, 'aaTrackball: center must be a two-dimensional array.'
        RETURN
    ENDIF

    IF (N_ELEMENTS(radius) NE 1) THEN BEGIN
        PRINT, 'aaTrackball: invalid radius.'
        RETURN
    ENDIF

    IF (N_ELEMENTS(axis) NE 0) THEN BEGIN
        IF (axis lt 0) OR (axis gt 2) THEN BEGIN
            PRINT, 'aaTrackball: invalid value for AXIS keyword.'
            RETURN
        ENDIF ELSE $
            self.axis = axis
    ENDIF ELSE $
    ...
    IF (N_ELEMENTS(mouse) NE 0) THEN BEGIN
        if (mouse ne 1) and (mouse ne 2) and (mouse ne 4) then begin
            PRINT, 'aaTrackball: invalid value for MOUSE keyword.'
            RETURN
        ENDIF ELSE $
            self.mouse = mouse
    ENDIF ELSE $
