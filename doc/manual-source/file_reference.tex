...
\begin{chapter}{\label{cha:file_reference}File reference}
  ...
  Table~\ref{tab:source_files} lists the \gpefile{.f90} source code files that
  ...
  general, it is recommended that \gpevar{nzprocs} $\geqslant$ \gpevar{nyprocs}
  ...
  Instead, copy one of the provided example \gpefile{Makefile.inc.*} files to
  \gpefile{Makefile.inc} and edit that.  Variables in this file may need to be
  ...
  variables to set in \gpefile{Makefile.inc}.
  %
  \begin{table}[ht]
    \centering
    \begin{tabular}{lp{0.67\textwidth}}
      ...
      \hline\hline
    \end{tabular}
    \caption{\label{tab:makefile}Description of \gpefile{Makefile.inc}
    variables.}
  \end{table}
