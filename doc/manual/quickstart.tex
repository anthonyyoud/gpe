...
\begin{chapter}{\label{cha:quickstart} Quick start guide}
  ...
  reduce any or all of \gpevar{nx}, \gpevar{ny}, or \gpevar{nz}.  (See also
  \gpevar{xr}, \gpevar{yr}, and \gpevar{zr} in section~\ref{subsec:runin}.)
  ...
  In the \gpeexample{ring} example, the initial condition is already correctly
  set to \gpevar{vortex\_ring(vr1)}, where \gpevar{vr1} is a type parameter
  declared in \gpefile{parameters.in}, which defines various aspects of the
  ...
  Finally, if numbers are appearing in some of the \gpefile{*.dat} files, for
  example, \gpefile{linelength.dat} or \gpefile{timestep.dat}, then you can at
